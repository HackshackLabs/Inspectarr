"""Serialize stale 4K movie snapshot for download (JSON, CSV, TXT, XML)."""
...
Stale4kMoviesSort = Literal["asc", "desc", "size_asc", "size_desc"]
...
def sort_stale_4k_movies(movies: list[dict[str, Any]], sort: Stale4kMoviesSort) -> None:
...
def _sorted_4k_movies(payload: dict[str, Any], sort: Stale4kMoviesSort) -> list[dict[str, Any]]:
...
def render_stale_4k_movies_export_json(payload: dict[str, Any], sort: Stale4kMoviesSort) -> bytes:
    movies = _sorted_4k_movies(payload, sort)
    out: dict[str, Any] = {
...
def render_stale_4k_movies_export_txt(payload: dict[str, Any], sort: Stale4kMoviesSort) -> bytes:
    lines: list[str] = []
    for m in _sorted_4k_movies(payload, sort):
...
def render_stale_4k_movies_export_csv(payload: dict[str, Any], sort: Stale4kMoviesSort) -> bytes:
    buf = io.StringIO()
    w = csv.writer(buf)
    w.writerow(
        [
...
def render_stale_4k_movies_export_xml(payload: dict[str, Any], sort: Stale4kMoviesSort) -> bytes:
    root = ET.Element("staleMovies4k")
...
def build_stale_4k_movies_export(
    fmt: ExportFormat,
    payload: dict[str, Any],
    sort: Stale4kMoviesSort,
) -> tuple[bytes, str, str]:
    if fmt == "json":
        return render_stale_4k_movies_export_json(payload, sort), "application/json", "stale-4k-movies.json"
    if fmt == "csv":
        return render_stale_4k_movies_export_csv(payload, sort), "text/csv; charset=utf-8", "stale-4k-movies.csv"
    if fmt == "txt":
        return render_stale_4k_movies_export_txt(payload, sort), "text/plain; charset=utf-8", "stale-4k-movies.txt"
    return render_stale_4k_movies_export_xml(payload, sort), "application/xml", "stale-4k-movies.xml"
