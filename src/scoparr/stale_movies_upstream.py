"""Live upstream (Tautulli / Radarr) progress while stale-movie snapshots build."""
...
def begin_stale_movies_upstream_trace(
...
def end_stale_movies_upstream_trace() -> None:
    now = int(time.time())
    with _lock:
        _state["busy"] = False
        _state["upstream"] = None
...
def record_stale_movies_tautulli(
    server_id: str,
    server_name: str,
    cmd: str,
...
def bump_stale_movies_tautulli_history_rows(server_id: str, server_name: str, delta: int) -> None:
    now = int(time.time())
...
def record_stale_movies_radarr(label: str, http_status: int, ok: bool) -> None:
    now = int(time.time())
    with _lock:
        _state["updated_epoch"] = now
