...
from tautulli_inspector.aggregate import canonical_utc_epoch_for_row
...
class TautulliClient:
    ...
    async def fetch_all_history(
        ...
        async with httpx.AsyncClient(timeout=self.timeout_seconds) as client:
            semaphore = asyncio.Semaphore(self.max_parallel_servers)
            tasks = [
                self._fetch_history_page_limited(
                    client=client,
                    semaphore=semaphore,
                    server=server,
                    start=start,
    ...
    async def fetch_all_tv_inventory(
        self,
        servers: list[TautulliServer],
        max_shows_per_server: int = 300,
    ) -> list[InventoryFetchResult]:
        """Fetch TV library inventory (shows, seasons, episodes) per server."""
        async with httpx.AsyncClient(timeout=self.timeout_seconds) as client:
            semaphore = asyncio.Semaphore(self.max_parallel_servers)
            tasks = [
                self._run_with_limits(
                    semaphore=semaphore,
                    coro=self._fetch_tv_inventory(
                        client=client,
                        server=server,
                        max_shows=max_shows_per_server,
                    ),
                )
                for server in servers
            ]
            return await _gather_safe(
                tasks=tasks,
                fallback_factory=lambda err: InventoryFetchResult(
                    server_id="unknown",
                    server_name="Unknown",
                    status="internal_error",
                    error=str(err),
    ...
    async def fetch_all_tv_inventory_chunk(
        self,
        servers: list[TautulliServer],
        batch_shows_per_server: int,
        get_next_start: Callable[[str, str], int],
    ) -> list[InventoryFetchResult]:
        """Fetch one incremental inventory chunk per TV section for each server."""
        async with httpx.AsyncClient(timeout=self.timeout_seconds) as client:
            semaphore = asyncio.Semaphore(self.max_parallel_servers)
            tasks = [
                self._run_with_limits(
                    semaphore=semaphore,
                    coro=self._fetch_tv_inventory_chunk(
                        client=client,
                        server=server,
                        batch_shows=max(batch_shows_per_server, 1),
                        get_next_start=get_next_start,
                    ),
                )
                for server in servers
            ]
    ...
    async def _fetch_tv_inventory(
        ...
        started_at = perf_counter()
        try:
            libraries_payload = await self._fetch_cmd_payload(client, server, "get_libraries")
            libraries = libraries_payload.get("response", {}).get("data", [])
            if not isinstance(libraries, list):
                libraries = []
            tv_sections = [lib for lib in libraries if str(lib.get("section_type")) == "show"]
            ...
            for section in tv_sections:
                ...
                if not section_id:
                    continue
                shows = await self._fetch_library_shows(
                    client=client,
                    server=server,
                ...
                for show in shows:
                    ...
                    show_key = str(show.get("rating_key") or "")
                    if not show_key:
                        continue
                    ...
                    for season in seasons:
                        ...
                        season_key = str(season.get("rating_key") or "")
                        if not season_key:
                            continue
            ...
            logger.info(
                "Upstream tv inventory fetch succeeded",
                extra={
                    "server_id": server.id,
                    "server_name": server.name,
                    "elapsed_ms": _elapsed_ms(started_at),
    ...
    async def _fetch_tv_inventory_chunk(
        ...
        try:
            ...
            for section in tv_sections:
                ...
                if not section_id:
                    continue
                start = max(get_next_start(server.id, section_id), 0)
                payload = await self._fetch_cmd_payload(
                    client,
                    server,
                ...
                if not isinstance(page_rows, list):
                    page_rows = []
                records_total = _as_int_or_none(data.get("recordsFiltered")) or 0
            ...
            logger.info(
                "Upstream tv inventory chunk fetch succeeded",
                extra={
                    "server_id": server.id,
                    "server_name": server.name,
                    "elapsed_ms": _elapsed_ms(started_at),
                    ...
                    "season_count": len(all_seasons),
                    "episode_count": len(all_episodes),
                    "sections": len(section_progress),
                    "index_complete": overall_complete,
                },
            )
            return InventoryFetchResult(
                server_id=server.id,
    ...
    async def _fetch_library_shows(
        self,
        client: httpx.AsyncClient,
        server: TautulliServer,
        section_id: str,
        ...
        """Fetch paged show rows for one TV section."""
        rows: list[dict] = []
        page_size = min(100, max(max_shows, 1))
        start = 0
        while len(rows) < max_shows:
            payload = await self._fetch_cmd_payload(
                client,
                server,
                "get_library_media_info",
            ...
            records_filtered = _as_int_or_none(data.get("recordsFiltered")) or 0
            start += len(page_rows)
            if start >= records_filtered or len(page_rows) < page_size:
                break
        return rows[:max_shows]

    async def _fetch_cmd_payload(
        self,
        client: httpx.AsyncClient,
        server: TautulliServer,
        cmd: str,
