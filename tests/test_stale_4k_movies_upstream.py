"""Stale 4K movies upstream trace (Tautulli / Radarr progress)."""
...
from scoparr.stale_4k_movies_upstream import (
    ...
    stale_4k_movies_upstream_snapshot,
)


class Stale4kMoviesUpstreamTests(unittest.TestCase):
    def tearDown(self) -> None:
        end_stale_4k_movies_upstream_trace()

    def test_radarr_movie_list_count_in_snapshot(self) -> None:
        begin_stale_4k_movies_upstream_trace()
        record_stale_4k_movies_radarr("GET /api/v3/movie", 200, True)
        set_stale_4k_movies_radarr_movie_list_count(42)
        s = stale_4k_movies_upstream_snapshot()
